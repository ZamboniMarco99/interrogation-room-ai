...
# Run all tests
test:
  pdm run pytest --cov=src/interrogation_room_ai --cov-report=term-missing --cov-report=html
...
[private]
  ...
  if ! gh repo list ZamboniMarco99 | grep --quiet interrogation-room-ai; then
    while true; do
      ...
      case $REPLY in
        [Yy])
          ...
          case $PRIVATE in
            [Yy])
              gh repo create ZamboniMarco99/interrogation-room-ai --private --source=.
              ;;
            [Nn])
              gh repo create ZamboniMarco99/interrogation-room-ai --public --source=.
              ;;
            *)
    ...
    while true; do
      ...
      case $REPLY in
        [Yy])
          git remote add origin git@github.com:ZamboniMarco99/interrogation-room-ai
          break
          ;;
        [Nn])
          exit 0
